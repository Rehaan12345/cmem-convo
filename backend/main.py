...
async def _run_member_seed(job_id: str, member_id: str, name: str, district: str,
    ...
    try:
        # Step 1: parse the PDF for council file IDs
        job["status"] = "parsing"
                ...
                for line in text.splitlines():
                    ...
                    if m:
                        ...
                        if fid not in titles:
                            file_ids.append(fid)
                            # grab the rest of the line as a rough title
        ...
        # Step 2: create/wipe the ChromaDB collection
        ...
        # Step 3: download + parse + index each council file
        total = len(file_ids)
        completed = 0
        total_chunks = 0
        failed: list[str] = []
        ...
        seed_log.info("=== Seed complete: %s ===", job["message"])
